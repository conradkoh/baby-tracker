...
import { useStopwatch } from '../../atoms/Stopwatch/useStopwatch';
...
export function BreastTimer(props: BreastTimerProps) {
  const { onStop } = props;
  const left = useStopwatch();
  const right = useStopwatch();
  const viewDuration = useCallback(
    (side: BreastSide) => {
      const sw = side === BreastSide.Left ? left : right;
      return sw.elapsed.toFormat('hh:mm:ss');
    },
    [left, right]
  );
  ...
  const stopped = useCallback(
    (side: BreastSide) => {
      const leftSeconds = parseInt(left.elapsed.toFormat('ss'));
      const rightSeconds = parseInt(right.elapsed.toFormat('ss'));
      onStop({
        totalDuration: {
    ...
    [left.elapsed, onStop, right.elapsed]
  );
  return (
    <View className="flex-row items-center pt-3">
      <View className="flex-1 items-center">
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (left.isActive) {
              stopped(BreastSide.Left);
            }
            left.toggle();
          }}
        >
          <Text>{!left.isActive ? '▶️' : '⏹️'}</Text>
        </TouchableOpacity>
      </View>
      <View className="flex-1 items-center">
        <View className="p-2">
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (right.isActive) {
              stopped(BreastSide.Right);
            }
            right.toggle();
          }}
        >
          <Text>{!right.isActive ? '▶️' : '⏹️'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
